...
import { formatPrice, resolvePaintingImageSrc } from "@/lib/paintings";
...
export function PaintingCard({ painting, compact }: Props) {
  ...
  function handleAdd(type: "original" | "print") {
    const ok = addItem({ ...painting, image: imageSrc }, type);
    setFeedback(ok ? "Ajouté" : "—");
    ...
    <article className="group">
      ...
      <div className="mt-4">
        <p className="text-xs uppercase tracking-[0.1em] text-stone-500">
          ...
          {sold && canPrint ? " · Tirages disponibles" : ""}
        </p>
        ...
        <div className="mt-4 flex gap-2">
          ...
          {!sold && canPrint && (
            <button
              type="button"
              onClick={() => handleAdd("print")}
              className="border border-stone-300 px-3 py-2.5 text-xs uppercase tracking-wider text-stone-700 hover:border-amber-800"
              title="Ajouter un tirage"
            >
              Tirage
            </button>
          )}
        ...
        {feedback && (
          <p className="mt-2 text-xs text-amber-900" aria-live="polite">
            {feedback}
          </p>
        )}
      </div>
